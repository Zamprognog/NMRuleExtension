...
We refer to \cite{wu_rule_2023} for a extended analysis of rule learning methods which we do not include in this work: specifically, beyond our analyzed methods, it includes a survey of deep-learning-based methods for RM . For the scope of this paper, we focused on the most directly interpretable and tunable models, but we consider the set of mined rules as an input for our analysis, so any of the reviewd methods should in principle be applicable.
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.12}
\caption{Materialization metrics for AnyBURL (with and without non-monotonic rules), with dis-aggregated inconsistency counts.}
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.12}
\caption{Materialization metrics for AMIE (with and without non-monotonic rules), with dis-aggregated inconsistency counts.}
...
\paragraph{Datasets.} We extract rules from the following Knowledge Graphs, which are selected as they come with a defined schema, summarized in Table \ref{tab:datasets}. 
...
\subsubsection{Inverse functionality analysis}
...
We thus generated YAGO4.5-10-flip, a variant of YAGO4.5-10 in which the usable functional properties are replaced by their inverse. The schema of YAGO4.5-10 declares four functional properties, all of which are perfectly functional in the data, but only two of them are usable for this purpose: \texttt{schema:gender} has nine distinct objects and, once inverted, would return a candidate set of the size of the whole \texttt{Person} population for every query, while \texttt{schema:publisher} occurs in 16 triples and declares blank-node domain and range, which carry no enforceable constraint. We therefore flip \texttt{schema:birthPlace} and \texttt{schema:deathPlace}, which together account for 196,162 triples of the dataset (140,221 and 55,941 respectively).

The construction is a purely directional rewriting: every triple $(s,P,o)$ becomes $(o,P^{-1},s)$ in the training, validation and test splits as well as in the full graph used for materialization, so that the number of triples, the entities and the splits are left unchanged (3,222,052 / 16,273 / 16,273). The inverse properties are minted in the YAGO namespace (\texttt{yago:birthPlaceOf}, \texttt{yago:deathPlaceOf}), as schema.org does not define these terms, and the entity type assertions are copied unchanged, since flipping alters the direction of a statement and not the entities involved. In the T-Box, the axioms of the original property are dropped and the inverse is declared an \texttt{owl:InverseFunctionalProperty} with domain and range \emph{swapped} with respect to the original. We deliberately do not assert an \texttt{owl:inverseOf} link between the two: the original property no longer occurs in the data, and declaring the link would reintroduce precisely the kind of twin relation that a rule learner can invert at no cost, confounding the measurement. For the same reason, the control condition for this analysis is the original YAGO4.5-10, which contains the same facts over the same entities and keeps domain, range and data volume fixed, and differs only in the direction of the two properties.

We verified the resulting dataset before using it: the triple counts are conserved in each split, the two flipped properties are empirically inverse functional (no object is associated with more than one subject), no occurrence of the original properties is left in the data or in the mined rules, and the flipped triples produce no domain or range violation when checked against the type assertions through the \texttt{subClassOf} closure of the schema. Finally, both properties are returned as inverse functional by the same Jena reasoning path used by our constraint loader, while the original schema returns none, confirming that the constraint is actually visible to the grounding engine.

Rule mining requires one adjustment with respect to the setup described above. Mining the flipped graph with the standard AnyBURL configuration yields 19,490 rules, of which only 104 and 58 have, respectively, \texttt{birthPlaceOf} and \texttt{deathPlaceOf} as head; their confidence is too low for them to survive the top-1\%, 5\% and 10\% cuts used in the materialization experiments, so that the two constrained relations are almost absent from the inferred triples (258 occurrences over the 5.9M triples inferred in the graph-wide run, all of them at the 10\% threshold). We therefore mined a second rule set restricted to the two inverse-functional heads, lowering the confidence threshold to 0.01 and leaving the remaining parameters unchanged, obtaining 20,562 rules that all target an inverse-functional relation. This guarantees that every materialized triple of this analysis is subject to the constraint under study. The probe remains limited to two constrained relations and is therefore meant to complement, and not to replace, the experiments on the four main datasets.
%%%%%%%%
\subsection{Results}
